...
export const useCreateChat = async (contactUid: string, uid: string) => {
  const chatRef = ref(db, '/allChats');

  const userRef = ref(db, `/users/${uid}/chats`);
  const contactRef = ref(db, `/users/${contactUid}/chats`);

  // create chatLog in general chats
  const chatObj = {
    participants: [uid, contactUid],
    chatlog: {},
  };

  // creating chat obj with log and participants
  const chatChildref = push(chatRef, chatObj);

  const chatChildKey = chatChildref.key;

  // pushing chatkey to contact and user
  push(userRef, chatChildKey);
  push(contactRef, chatChildKey);
